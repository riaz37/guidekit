...
export async function handleLLMProxy(
  ...
  if (!permissions.includes('llm')) {
    return jsonResponse({ error: 'Permission "llm" not granted' }, 403);
  }
  ...
  if (typeof body.systemPrompt !== 'string' || body.systemPrompt.length === 0) {
    return jsonResponse({ error: 'Invalid body: systemPrompt must be a non-empty string' }, 400);
  }
  if (!Array.isArray(body.contents)) {
    return jsonResponse({ error: 'Invalid body: contents must be an array' }, 400);
  }
  if (body.userMessage !== undefined && typeof body.userMessage !== 'string') {
    return jsonResponse({ error: 'Invalid body: userMessage must be a string' }, 400);
  }
