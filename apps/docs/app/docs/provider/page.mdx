...
description: Configure GuideKitProvider — token endpoint, LLM proxy, agent persona, and widget options.
---
...
Use `siteKnowledge` when the assistant should answer from indexed content across the website, not only the current page. The endpoint is served by `@guidekit/server` and requires a valid GuideKit session token with `site:read`.

```tsx
<GuideKitProvider
  tokenEndpoint="/api/guidekit/token"
  proxy={{ llm: '/api/guidekit/llm' }}
  llm={{ provider: 'gemini' }}
...
Applies to the **built-in widget only** (when `headless` is not set). For full UI control, use [Custom UI (Headless)](/docs/custom-ui).
...
Set `headless` to skip the built-in widget and build your own UI with hooks:
...
See [Custom UI (Headless)](/docs/custom-ui) for patterns, positioning, and consent.
...
|------|------|---------|-------------|
| `headless` | `boolean` | `false` | Skip built-in widget; use hooks for custom UI |
