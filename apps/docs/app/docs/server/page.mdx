    ...
    documents: [
      {
        id: 'pricing',
        title: 'Pricing',
        content: 'The Pro plan includes guided autonomy and priority support.',
...
// app/api/guidekit/site-search/route.ts
export const POST = routes.POST_siteSearch;
```
...
// Route dispatch: 'token' | 'llm' | 'health' | 'stt' | 'tts' | 'site-search'
...
| `/site-search` | POST | Search server-backed website knowledge |
