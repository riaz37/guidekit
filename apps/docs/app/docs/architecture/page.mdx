...
- **Budget**: 5000-node limit, yields every 5ms via `requestIdleCallback`
...
- **Provider adapters**: Gemini (default), OpenAI format
...
```
Mic → VAD (Silero) → STT (Deepgram) → LLM (Gemini) → TTS (ElevenLabs) → Speaker
```
...
| TTS | ElevenLabs Flash v2.5 (WebSocket) |
