...
const { hasConsent, grantConsent, revokeConsent } = useGuideKitConsent({
  ...
  instanceId: 'default', // optional, matches GuideKitProvider instanceId
});
```
...
See [Custom UI (Headless)](/docs/custom-ui) for a full consent dialog pattern.
...
| `useGuideKitConsent` | Consent state changes |
